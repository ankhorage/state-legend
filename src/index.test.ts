...
it('createLegendStateAdapter supports path-based set and get', () => {
...
it('createLegendStateAdapter supports initial state and string paths', () => {
  const adapter = createLegendStateAdapter({
    initialState: {
      session: {
        user: {
...
it('createLegendStateAdapter notifies subscribers on set', () => {
  const adapter = createLegendStateAdapter();
...
it('createLegendStateAdapter stops notifying after unsubscribe', async () => {
  const adapter = createLegendStateAdapter();
  const snapshots: unknown[] = [];
  const subscriptionResult = adapter.subscribe('counter.value', ({ value }) => {
    snapshots.push(value);
...
it('createLegendStateAdapter deletes values and notifies subscribers', () => {
  const adapter = createLegendStateAdapter({
    initialState: {
      session: {
        user: {
...
it('createLegendStateAdapter returns deterministic errors for empty paths', () => {
  const adapter = createLegendStateAdapter();
...
it('createLegendStateAdapter returns deterministic errors for nested path conflicts', () => {
  const adapter = createLegendStateAdapter({
    initialState: {
      session: 'invalid',
    },
